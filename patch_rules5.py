...
old_rule = """
    match /active_sessions/{sessionId} {
      allow read: if isAuthenticated() && (resource == null || resource.data.userId == request.auth.uid) && sessionId.matches('^' + request.auth.uid + '_.*');
      allow read: if isAdmin();
      allow create: if isAuthenticated() && request.resource.data.userId == request.auth.uid && sessionId.matches('^' + request.auth.uid + '_.*');
      allow update, delete: if isAuthenticated() && resource.data.userId == request.auth.uid && sessionId.matches('^' + request.auth.uid + '_.*');
