...
変わる
かわる (berubah, perubahan, berubah, berbeda, beralih)
...
while i < len(lines):
    # Sometimes it's a single line like: 旅館 (rumah penginapan model jepang) / りょかん
    # We will look ahead
    line1 = lines[i]
    if i + 1 < len(lines) and '(' in lines[i+1]:
        ...
        if '(' in line2:
            ...
            if key not in seen:
                ...
                vocab_list.append({
                    "jp": kanji,
                    "romaji": hiragana,  # Storing hiragana in 'romaji' field
                    "id_translation": meaning,
                    "category": "JFT_A2_1_50"
                })
        i += 2
    else:
        # Check if line1 has it all "旅館 (rumah penginapan model jepang)" and line2 is "りょかん"
...
# Let's break them into chunks if needed, or save all in one file
with open('src/data/jft_a2_1_50.json', 'w') as f:
    json.dump(vocab_list, f, indent=2, ensure_ascii=False)

print(f"Extracted {len(vocab_list)} unique words.")
